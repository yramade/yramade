...
CREATE TABLE IF NOT EXISTS Game (
  ...
  choice ENUM('strong', 'weak'), -- 초이스 결정: 'strong'(강한 패 승) 또는 'weak'(약한 패 승)
  choiceBy ENUM('player', 'ai'), -- 초이스 선택자: 'player' 또는 'ai'
  winner ENUM('player', 'ai', 'draw'), -- 승자 정보
  playerDice TINYINT UNSIGNED, -- 플레이어 주사위 결과 (1~6)
  aiDice TINYINT UNSIGNED,         -- AI 주사위 결과
  createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,         -- 게임 생성 시각
  PRIMARY KEY (`id`),
  FOREIGN KEY (userId) REFERENCES User(id),              -- 사용자 테이블 참조
  CHECK (playerDice BETWEEN 1 AND 6),
  CHECK (aiDice BETWEEN 1 AND 6)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin;
...
CREATE TABLE IF NOT EXISTS GameLog (
  ...
  eventType ENUM('deal', 'exchange', 'roll', 'choice', 'showdown'), -- 주사위, 초이스, 쇼다운 이벤트 종류
